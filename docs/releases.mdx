...
:::
## V2.1.4.0 (Mar 01, 2022)

#### Important Fixes

* Fixed an issue with entering values in manual simulation.

## V2.1.3.0 (Feb 22, 2022)

#### Important Fixes

* Fixed an issue with allowing edit on cells kept on hold.
* Increased session time to login to the workspace.

## V2.1.2.0 (Feb 16, 2022)

#### Important Fixes

* Users can now refer to any node from another model having multiple models merged to it.

  <div class="center">
    <Zoom>
      <img
        alt="Model Merge"
        src={useBaseUrl("/doc-images/model/multilevelmm.gif")}
      />
    </Zoom>
  </div>
* Fixed an issue with entering values in Plan tab for single time period.

## V2.1.1.0 (Feb 8, 2022)

#### Important Fixes

* Title prefix in node configuration restricts users not to enter more than 10 characters.
* Fixed an issue with downloading Variance Analysis report from visual.
...
* Selections made in <Link to={useBaseUrl("/analysis/attribution-analysis/")}>Attribution analysis</Link> will now be retained while switching tabs
 
  <div class="center">
    <Zoom>
      <img
        alt="Selection Retention"
        src={useBaseUrl("/doc-images/analyze/AA-retain selection.png")}
      />
    </Zoom>
  </div>

* Trend <Link to={useBaseUrl("/simulation/simulation_canvas/#iii-line-and-column-chart")}>chart</Link> is removed from Node details for models with single period
 
  <div class="center">
    <Zoom>
      <img
        alt="Trend Chart"
        src={useBaseUrl("/doc-images/model/NodeDetail-singlePeriod.png")}
      />
    </Zoom>
  </div>

* <Link
    to={useBaseUrl("/analysis/sensitivity-analysis/#download-report-as-excel")}>
    Sensitivity Analysis
  </Link> download file formatting has been updated by adding headers, border and
  better alignment for improved reabability

* <Link to={useBaseUrl("/model/node/#1-general")}>Title prefix</Link> outside tree is now
available across all tabs where tree hierarchy is not present

  <div class="center">
    <Zoom>
      <img
        alt="Model Merge"
        src={useBaseUrl("/doc-images/model/titleprefixoutsidetree.gif")}
      />
    </Zoom>
  </div>

* Added <Link to={useBaseUrl("/model/node/#slide-right-to")}>Slide Right</Link> to option in Quick Editor where you can choose to increase or decrease the node value when the slider is moved in the right direction
* Added model merge icon for nodes linked from another model in Plan tab
  ...
  <div class="center">
    <Zoom>
      <img
        alt="Model Merge"
        src={useBaseUrl("/doc-images/model/mm-icon.png")}
      />
    </Zoom>
  </div>

* Node search gets closed when user clicks outside the search drop down
* Added Simulation configuration in Edit node for model merge nodes
 
  <div class="center">
    <Zoom>
      <img
        alt="Model Merge"
        src={useBaseUrl("/doc-images/model/mm-simulation.gif")}
      />
    </Zoom>
  </div>

* <Link to={useBaseUrl("/simulation/navigation_panel/#scenarios")}>
    Compared scenarios
  </Link> selection will now be pre-selected for download
* Fixed an issue with the formula pop up in Quick Editor which didn't allow users to enter formula
* Added Title prefix as a customization option in Export as datasource
  
  <div class="center">
    <Zoom>
      <img
        alt="Title prefix as a customization"
        src={useBaseUrl("/doc-images/export-customization.png")}
      />
    </Zoom>
  </div>

* Removed nodes having Calculation Method as<Link to={useBaseUrl("/model/node/#22-calculation-method ")}> Linked to node </Link> for node level model merge

#### Important Fixes

* Using <Link to={useBaseUrl('/planning/plan_features/#39-refresh-datasource')} >Refresh datasource</Link> option in main model now brings the updated values from sub model

  <div class="center">
    <Zoom>
      <img
        alt="Refresh   Datasource"
        src={useBaseUrl("/doc-images/plan/refreshdatasource.gif")}
      />
    </Zoom>
  </div>

* Fixed an issue where users can now use Model > General > <Link to={useBaseUrl("/model/general/#1-export")}>Export</Link> to get the export configuration without the model getting crashed
* Sub model nodes will be available in the level at which model merge has been performed while using <Link to={useBaseUrl("/storage/export/")}>export as datasource </Link>

  <div class="center">
    <Zoom>
      <img
        alt="Refresh   Datasource"
        src={useBaseUrl("/doc-images/model/mmlevel.gif")}
      />
    </Zoom>
  </div>
...
* <Link to={useBaseUrl("/model/node/#quick-editor-in-node-tab")}>Quick Editor</Link> allows
  selection of up to 2 nodes under Composite nodes column
  
  <div class="center">
    <Zoom>
      <img
        alt="Quick Editor"
        src={useBaseUrl("/doc-images/quickEditor/CompositeNode.gif")}
      />
    </Zoom>
  </div>
...
* Changes made in <Link to={useBaseUrl('/model/node/#export-to-excel')}>export</Link> csv will now reflect in Quick Editor on importing the excel file

## V2.0.9.1

#### Enhancements

* Up to 50 nodes are available in <Link to={useBaseUrl('/storage/model-merge/#case-2-linking-node-to-a-single-node-from-a-model')}>search</Link> while linking a node from another model

#### Important Fixes

* Model rendering issue has been fixed where the model kept perpetually loading
* Users can now edit the <Link to={useBaseUrl('/model/node/#2-configuration')}>configuration</Link> of nodes linked to another model
