...
* Sensitivity Analysis chart includes the <Link to={useBaseUrl('analysis/sensitivity-analysis#download-report-as-excel')}>Download</Link> button to export the SA report in an Excel.
<div class="center">
  <Zoom>
    <img
...
<div class="center">
  <Zoom>
    <img
      ...
      src={useBaseUrl("/doc-images/analyze/simulate-slide-right-to.png")}
    />
  </Zoom>
  <p>Set the node simulation right slider trend</p>
</div>
...
* Added Back to main model for an easy navigation to switch to the main model in both tree and table view.
...
:::important
Mention in SA module
:::
* Redesigned table view for better user experience
:::important
We need to have a few more points on this
:::
...
* Table view periods now gets updated based on Simulation period and Value Display selection made in Navigation Panel.

<div class="center">
  <Zoom>
    <img
      alt="Simulation period in table"
      src={useBaseUrl("/doc-images/simulate_tab/sim-period-impact-tableview.png")}
    />
  </Zoom>
  <p>Selected Simulation Period and Value Display reflected in table view</p>
</div>

* Added help ( <img width="25" src={useBaseUrl('img/help.png')}/> ) icon link against **Edit Node > Configuration > Calculation Method > Formula > Custom Formula** and **Edit Node > Secondary KPI > Custom Formula**. This link provides a quick access to Functions section of ValQ documentation.
* Added message on Plan tab and updated the tooltip message for Model tab while on Power BI Service's reading view asking users to switch to Edit mode.

<div class="center">
  <Zoom>
    <img
      alt="Reading view message"
      src={useBaseUrl("/doc-images/read-view-message.png")}
    />
  </Zoom>
  <p>Message and tooltip to switch to edit mode</p>
</div>
...
* Added Unique ID field under Customization section of the <Link to={useBaseUrl('storage/export')}>Export as datasource</Link> feature.
* Templated nodes gets automatically hidden in the Model tab on using it as Template based on other node
* Selecting a node in the plan series shows lineage upto last 3 levels in the footer
<div class="center">
  <Zoom>
    <img
...
* **Model > New** and **Model > General > Writeback** configuration is now restricted only to the Model Owner for enhanced security.

#### Important Fixes

* Paste option from right click has been removed for nodes that are linked to another model from workspace
* Export as datasource for Tree structure now exports the merged model with nodes at proper level
* Sub model section in Navigation Panel does not duplicate Top node when using Linking to Node in the model under Link to workspace model
...
* Fixed an issue where a node linked to workspace model didn't update the values when changing baseline series after refreshing the scenario
...
* Color for Variance values on Input node tooltip of Sensitivity Analysis now takes into consideration the desired trend of the Output node
* Reset all now works for Rolling Forecast
-->
## V2.0.6 (Jun 4, 2021)
