...
1. The **Plan** tab includes <Link to={useBaseUrl('planning/plan_features#7-writeback')}>Writeback</Link> feature that triggers to send data to an external API.
...
1. Planning series implements single lock icon as a toggle to <Link to={useBaseUrl('planning/plan_features#33-hold-and-release')}>Hold/Release</Link> the cells.
1. Plan/Simulation/Variance Analysis tab introduces <Link to={useBaseUrl('planning/plan_features#6-size-columns-to-fit')}>Size columns to fit</Link> option to auto-size the columns width.
1. Added <Link to={useBaseUrl('simulation/navigation_panel#show-composite-nodes')}>Show Composite Nodes</Link> as a toggle in Navigation Panel for models which have Composite Nodes.
1. The <Link to={useBaseUrl('model/node#manage-columns-settings')}>Manage Columns</Link> settings in the **Model > Node > Quick Editor** is grouped similar to the labels in the **Edit Node** configurations.
