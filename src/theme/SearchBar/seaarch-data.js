export default [
  {
    ...
    content:
      "Welcome to this blog. This blog is created with Docusaurus 2 alpha.This is a test post.A whole bunch of other information."
  },
  {
    title: "Hola",
    type: 0,
  ...
  },
  {
    ...
    url: "blog/welcome",
    content:
      "Blog features are powered by the blog plugin. Simple add files to the blog directory. It supports tags as well!Delete the whole directory if you don't want the blog features. As simple as that!"
  },
  {
    title: "Variance Analysis",
    type: 0,
    sectionRef: "#",
    url: "docs/analysis/variance-analysis",
    content:
      "The Analyze Tab allows the user to run advanced analyses on the data. With 1.4, Variance Analysis has been introduced. Several other features such as Sensitivity Analysis and Time Series Analysis are to be released in the upcoming versions.Variance Analysis displays the variance between the Base and Compared series across all nodes in the model.On clicking the 'Analyze' Tab, you will be able to view the Variance Analysis option.The Variance Analysis page is displayed, showing the variance between the Base and Compared data series.The series that were created in the 'Plan' Tab can also be selected as Base Series or Compared Series by clicking on the drop-down.The period across which the two series are to be compared can be customized by clicking on the drop-down in the 'Period' field.Variance analysis can also be performed for the two series across different periods by selecting the 'Compare to different period' option and choosing the period in the drop-down.It is to be noted that Base Series and Compared Series cannot be same when comparing for the same Base Period. However, when comparing across different periods Base Series and Compared Series can be same. The 'Filter' option has the ability to filter the nodes based on the 'Variance' values. The number of nodes and the period can be selected by the user.The user can also choose to display either Absolute Variance or % Variance or both by selecting the appropriate checkboxes.It is possible to download the results of the analysis as an Excel file when using Power BI Service.The downloaded analysis looks like the one shown below."
  },
  {
  ...
  },
  {
    ...
    content:
      "#H3 - Create the best documentation #H4 - Create the best documentation #H5 - Create the best documentation #H6 - Create the best documentation  "
  },
  {
    title: "Emphasis",
    type: 1,
  ...
  },
  {
    ...
    url: "docs/doc1#code",
    content:
      "var s = 'JavaScript syntax highlighting';alert(s);Copy s = \"Python syntax highlighting\"print sCopy No language indicated, so no syntax highlighting.But let's throw in a <b>tag</b>.Copy function highlightMe() { console.log('This line can be highlighted!');}Copy  "
  },
  {
    title: "Tables",
    type: 1,
    ...
    content:
      "Colons can be used to align columns. TablesAreCoolcol 3 isright-aligned$1600col 2 iscentered$12zebra stripesare neat$1 There must be at least 3 dashes separating each header cell. The outer pipes (|) are optional, and you don't need to make the raw Markdown line up prettily. You can also use inline Markdown. MarkdownLessPrettyStillrendersnicely123  "
  },
  {
    title: "Blockquotes",
    type: 1,
  ...
  },
  {
    ...
    type: 0,
    sectionRef: "#",
    url: "docs/faq/definitions",
    content:
      "Power BIMicrosoft Power BIvalQvalQ for Microsoft Power BIValue Driver TreeMathematical or conceptual business models visualised in a way that links the business KPI’s (what management or stakeholders care about) to the operational drivers (the things that can be influenced to change the KPI’s).NodeA generic reference to a unique KPI or Value Driver in a value driver treeNode WidgetThe visualization of a KPI or Value Driver in a value driver treeDerived NodeAdditional Information for a node but not directly related to the calculation hierarchy. This will often be key benchmarking KPI’s such as cost/production unit.ParentA reference to the Node one level above in the Value Driver TreeChildrenA reference to the collection of nodes immediately below a NodeDefinitions"
  },
  {
    title: "List of Formulas and Functions",
    type: 0,
    sectionRef: "#",
    url: "docs/faq/formula-list",
    content: ""
  },
  {
  ...
  },
  {
    ...
    content:
      "NAME SYNTAX DESCRIPTION Range .Range(start_index,end_index) LastNPeriods .LastNPeriods(n) Get .Get(period_index) ForEach ForEach(Array,Iterator) Iterator function "
  },
  {
  ...
  },
  {
    ...
    url: "docs/mdx",
    content:
      "You can write JSX and use React components within your Markdown thanks to MDX. Docusaurus green and Facebook blue are my favorite colors.I can write Markdown alongside my JSX!"
  },
  {
    title: "Data Series",
    type: 0,
    sectionRef: "#",
    url: "docs/model/data-series",
    content: ""
  },
  {
    title: "Data Series Manager",
    type: 1,
    pageTitle: "Data Series",
    url: "docs/model/data-series#data-series-manager",
    content:
      'Using the option "Data Series Manager", you will be able to configure the Periods and Data Series for the Tree (see Figure below).  For our example, we have the data source having periods from Jan to Dec, Budget and Forecast values. The Budget values will be the Comparison values and the Forecast values will be the Baseline values. Follow the below steps for configuring the values in the Data Series Manager. Set the property Period(s) per data series to the value 5.Configure the Period Labels as Jan, Feb, Mar, Apr, May (see Figure above).Enable the property Include comparison data series. You can view both the Baseline and Comparison values.When the property Include comparison data series is disabled, then you will be able to view only the Baseline Series.Since our data set has the Additional values Additional 1 and Additional 2, it will be displayed as Additional Series (see Figure above).When you click the Baseline label, you will be able view the Baseline values as Forecast values as shown below.  When you click the Comparison label, you will be able view the Comparison values as the Budget values as shown below  Based on the above set of configuration steps, you will be able to view the valQ screen as shown below.  You can observe from the above Figure that after simulating the Node Sale Allowances, you will be able to view the Baseline data in comparison with simulated Baseline data. '
  },
  {
    title: "Time Aggregation Labels",
    type: 1,
    pageTitle: "Data Series",
    url: "docs/model/data-series#time-aggregation-labels",
    content:
      "Using the option “Time Aggregation Labels”, you will be able to configure the Active Period, Till Prior Period, All Periods and Simulation Period settings (see Figure below).  Based on the above configuration, you will be able to view the valQ screen as shown below.  From the above Figure, you will be able to view the Time Aggregation Labels as highlighted in the valQ screen. "
  },
  {
    title: "Data Sorting",
    type: 1,
    pageTitle: "Data Series",
    url: "docs/model/data-series#data-sorting",
    content:
      "Using the Data Sorting settings, you will be able to view and customize your Data Source. You will be able to select the Sort fields, sort the data with Ascending or Descending Order and select the Start With field with the required Month (see Figure below).  "
  },
  {
    title: "Additional Properties of Data Series Tab",
    type: 1,
    pageTitle: "Data Series",
    url: "docs/model/data-series#additional-properties-of-data-series-tab",
    content:
      "AreaPropertyDescriptionData Series ManagerPeriodsPeriod(s) per data series: It indicates the number of values in the series. As an example, the series with Jan, Feb and Mar can be entered as 3 periods.Period Label(s): The Labels can be given as Jan, Feb, Mar (being separated by commas.Data SeriesBy enabling this property, you will be able to include the Comparison Data Series.The Preview shows all the available Data Series (both Baseline and Comparison) and the Additional Series.Time Aggregation LabelsThese labels are used in the navigation panel, nodes and popup screens.Active periodThis property sets only one period of interest. A Label and a three letter abbreviation can be provided for this property.Till Prior PeriodThis property sets Total value from beginning to a specific period. A Label and a three letter abbreviation can be provided for this property.All periodsThis property sets Total value of all periods in the series. A Label and a three letter abbreviation can be provided for this property.Simulation PeriodThis property sets a specific set of continuous periods. A Label and a three letter abbreviation can be provided for this property.Data SortingThe data can be sorted using the below filters: 1. Choose Sort Field(s) from the data source 2. Sort By: Ascending/Descending 3. Start with entry from the data source Additional Properties of Data Series Tab "
  },
  {
    title: "General",
    type: 0,
    sectionRef: "#",
    url: "docs/model/general",
    content: ""
  },
  {
    title: "Export",
    type: 1,
    pageTitle: "General",
    url: "docs/model/general#export",
    content:
      "In the Export settings, you will be able to configure the export functionality. There is also an option to include the navigation settings panel in the Export File (see Figure below).  The saved file can be exported and this exported file can be used in the “Import from Excel” option in “Import an Advanced Model” tab (Refer Section: Import from an Export File <IEF>). "
  },
  {
    title: "Writeback",
    type: 1,
    pageTitle: "General",
    url: "docs/model/general#writeback",
    content:
      "You will be able to export the Scenarios configured in the valQ through an URL by entering the Writeback URL in the Text Box (see Figure below).  The different simulations of planning results can be written back to the underlying systems using the URL exposed as part of the valQ. Using the URL, the user can do write-back scenario nodes to the underlying system. For more details on Writeback Functionality, you can refer the link: https://valq.com/blogs/writeback-valq-data-using-microsoft-flow "
  },
  {
    title: "Additional Properties of the General Tab",
    type: 1,
    pageTitle: "General",
    url: "docs/model/general#additional-properties-of-the-general-tab",
    content:
      "AreaPropertyDescriptionExportInclude navigation panel settings in the export fileThis property when activated exports the file along with Navigation Panel settings.WritebackExport Scenarios via Writeback URLYou will be able to export the Scenarios configured in the valQ through an URL by entering the Writeback URL in the Text Box. Additional Properties of General Tab "
  },
  {
  ...
  },
  {
    ...
    type: 1,
    pageTitle: "Settings",
    url: "docs/model/settings#number-formatting",
    content:
      'In the Number Formatting settings, you will be able to configure the following properties: In the area Scale Suffix, set the property Thousands to "k" (see Figure below).  Set the property Millions to "m" (see Figure above).Set the property Billions to "b".Set the property Trillions to "t".In the area Display Formatting, set the property Display zeroes as to the option "0".Set the property Display Negative values as to the option "(0)".Set the property Decimal Separator to ".".Set the property Thousands Separator to ",". Based on the above configuration, you will be able to view the valQ screen as shown below.  From the above Figure, you can observe that the zero value is displayed as "0" based on our configuration. The negative value has been represented as "(5,690)". The thousand separator is represented as 6,942. The scale suffix settings will be displayed based on the data set that we use. '
  },
  {
    title: "Conditional Formatting",
    type: 1,
    pageTitle: "Settings",
    url: "docs/model/settings#conditional-formatting",
    content:
      'In the Conditional Formatting settings, you will be able to set Alert Threshold and Rules. You will be able to configure the following properties for the Conditional Formatting. You can enable/disable the property Enable Conditional Formatting as shown in the below.  In the area Format based on, select the option Variance as shown in the above Figure. Now configure the Formatting Threshold values as highlighted in the above Figure. Based on the above settings, you will be able to view the Tree as shown below.  After simulation, you can observe that the variance percentage value for the Node "Income" falls in the Threshold range 22% which is above 0% and based on that condition, the status bar for the Node is green color (see Figure above). Similarly the variance percentage value for the Node "Investment and Others" falls in the Threshold range (26%)* which is below -10% and based on that condition, the status bar for the Node is red color.Now select the option Simulation as shown in the below Figure.  Based on the above settings, you will be able to view the Tree as shown below.  After simulation, you can observe that the simulation percentage value for the Node "Income" falls in the Threshold range 29% which is above 0% and based on that condition, the status bar for the Node is green color (see Figure above). Similarly the simulation percentage value for the Node "Investment and Others" falls in the Threshold range (37%)* which is below -10% and based on that condition, the status bar for the Node is red color. Note: * indicates the negative values. '
  },
  {
    title: "Color Theme",
    type: 1,
    pageTitle: "Settings",
    url: "docs/model/settings#color-theme",
    content:
      'In the Color Theme settings, you will be able to configure the following properties: You will be able to select the Light Theme or Dark Theme based on your choice (see Figure below). For our example, the Light Theme has been selected.  Set the property "Choose Navigational Panel Theme" to the option Dark.Set the Main and Accent color for the Canvas to the color as shown in the above Figure.For the Tree Widget, set the colors for the properties Font Color, Background, Derived Nodes, Highlighted and Connectors to the required colors as shown in the above Figure.You will be able to view the valQ screen having Light Theme color along with different colors selected for both the canvas as well as the tree widget based on the configured settings.  Also you can observe that the Navigation Panel will be shown in Dark Theme based on the configuration.  For our example, you have configured the Highlighted color for the Node as Red Color (see Figure below).  Based on the above configuration, you can observe that when you click and Node in the Navigation panel, the Node in the right pane will be highlighted based on the color selection. For our example, the Node is being highlighted in Red color.  '
  },
  {
    title: "Additional Properties of Settings Tab",
    type: 1,
    pageTitle: "Settings",
    url: "docs/model/settings#additional-properties-of-settings-tab",
    content:
      "The Navigation Panel in the valQ screen will be configured based on the below listed properties. AreaPropertyDescriptionNavigation PanelEnable Navigation PanelThis property enables/disables the Navigation Panel in the valQ screen.ScenariosYou can enable/disable the Visibility option for the Scenarios using the Eye icon and also set the Label for the Scenario using the Edit icon in the Navigation Panel Settings.Sub ModelsYou can enable/disable the Visibility option for the Sub Models using the Eye icon and set the Label for the Sub Models using the Edit icon in the Navigation Panel Settings. You can select the Nodes from Tree/Sub-Tree List. You can also enable/disable the property for filtering the list based on the selected sub tree.Simulation PeriodYou can enable/disable the Visibility option for the Simulation Period using the Eye icon and also set the Label for the Simulation Period using the Edit icon in the Navigation Panel Settings. You can also set the Simulation Period Start and Simulation Period End. When you need to start the simulation from the current month, then you can select the “Start to current Month” optionValue DisplayYou can enable/disable the Visibility option for the Value Display using the Eye icon and set the Label for the Value Display using the Edit icon. You can select the Primary Period, Scaling options and Default Scaling options for the Value Display in the Navigation Panel.Key InputsYou can enable/disable the Visibility option for the Key Inputs using the Eye icon and set the Label for the Key Inputs using the Edit icon. Further you can also select the Key Assumptions to be displayed as List by selecting the required Nodes in the Navigation PanelKPIsYou can enable/disable the Visibility option for the KPIs using the Eye icon, set the Label for the KPIs using the Edit icon and further select the KPIs to be displayed as List by selecting the required Nodes in the Navigation Panel.ConstraintsYou can enable/disable the Visibility option for the Constraints using the Eye icon, set the Label for the Constraints using the Edit icon and further select the Constraints to be displayed as List by selecting the required Nodes in the Navigation Panel. You can also enable/disable the property for sorting the constraints based on utilization.NodesData Mapping FormatThis Node settings allows you to map the Nodes to queries in the ID, Text or ID + Text Formats.Map based on ID or TextFor example, you can use this option if the field value exactly matches your Node’s unique ID or Text (e.g., Text - ‘Canada’)Map based on ID-Text PairFor example, you can use this option if your field has a key-text format (e.g., ‘CA:Canada’), and your Node’s ID corresponds to a key (e.g.,‘CA’).ID-Text Pair SeparatorYou can provide the inputs for the ID-Text Pair format.Default Node StyleThis property is used to configure the style of the Node. The options are Standard, Full and Minimal.Node ElementsThis property is used to display the below listed elements in the Node: 1. Variance 2. Trend Sparkline 3. Secondary Value/Variance 4. Descendant Node CountData Source ExceptionsBy enabling the option “Data Source Exception”, you can use the Data provided in “Manual” Calculation Method when the Data Source Method returns no value for the Node.Waterfall Chart TypeThe Pop up screen will be appearing with the type of the Waterfall Chart being selected. The options are Vertical and Horizontal.Number FormattingScale SuffixThis property sets the Thousands, Millions, Billions and Trillions Suffix for the scale.Display FormattingThis property sets the Number Formatting for the following fields: 1. Display zeroes as None, - or 0 2. Display Negative Value as ( 0), -0 or 0- 3. Decimal Separator as “.” 4. Thousand Separator as “,”CanvasDefault VisualThis property sets the Default Visual as Tree or Table in the right pane.Start the Model with the following KPIThe Model will get started based on the on the selection the specific Node from the list.Expand Hierarchy upto the following number of levels by defaultUsing this property, you can expand the hierarchy upto the following number of levels by default. The minimum level is 2.Footer Toolbar – Enable Node SearchThis property enables/disables the Node Search in the canvas.Footer Toolbar – Enable hiding empty nodesThis property hide Nodes that have no value mapped to it or have null value for all its periods.Default canvas zoom (%)This property sets the default zoom ratio for the canvas.Enable Conditional FormattingThis property enables/disables the Conditional FormattingDefault Table StyleThis property is used to configure the style of the Table. The options are Standard and Full.HintThis property sets the Hint which appears at the top of the canvas.Conditional FormattingEnable Conditional FormattingThis property enables/disables the Conditional Formatting property.Format based onEach node will have a status bar property to indicate whether the trend of values is favorable and this property can be configured using these options Variance and Simulation.Formatting Thresholds (%)The Status bar color will be changed based on the percentage of deviant from initial value. The color ranges are given below: 1. Red - Below:- 10% 2. Yellow - Between: -10% to-1% 3. White - Between: -1% to1% 4. Green - Above: 1%Color ThemeChoose Canvas ThemeThis Property sets the Theme for the Node. The options are Light Theme and Dark Theme.Choose Navigation Panel ThemeThis property sets the color for the Navigational Theme. The options are Dark and Light.CanvasThis property sets the Main Color and Accent Color for the Canvas.Tree widgetThis property sets the Font color, Background color, Derived Nodes color, Highlighted Node color and Node Connectors color for the Tree Widget. Additional Properties of Settings Tab "
  },
  {
  ...
  },
  {
    ...
    type: 1,
    pageTitle: "Budgeting & allocation scenarios",
    url:
      "docs/planning/budget-allocation#scenario-1-to-increase-total-revenue-by-10-and-allocate-the-increase-to-sales-revenue-with-no-changes-to-other-revenue",
    content:
      "For our example, expand the Total Revenue. Now select the node 'Other Revenue' and lock the node (see Figure below).Now select the Total Cell for node 'Total Revenue' as shown in the below Figure.You can update the number by double clicking the cell and entering the number directly in the number bar. Or you can enter the number in the popup.Now double click the total cell and type '+10%' at the end of the number and click on 'Weight' as shown in the below Figure.You will be able to view the 'Spread by Weights' screen.By default, the current node and the current series are selected. You can also select different weights to allocate. You can further decide to allocate the full amount or the delta amount (see Figure above).The 'Overwrite Values' option deletes the existing period values and rewrites the new allocated value. The 'Append to Values' option will add the delta value to the existing period values and only the delta value gets allocated based on the selected weights (see Figure above). For our example, the Overwrite option has been selected.As an end result of the Scenario 1, you will be able to view the Input Editor screen updated with the configured values (see Figure below). The cells which were impacted because of the allocation operations turns yellow. "
  },
  {
    title: "Scenario 2: To increase 'Cost of Sales' by 10% from April onwards",
    type: 1,
    pageTitle: "Budgeting & allocation scenarios",
    url:
      "docs/planning/budget-allocation#scenario-2-to-increase-cost-of-sales-by-10-from-april-onwards",
    content:
      "For our example, select the Node 'Cost of Sales' for April month a. (see Figure below).Increase the cell value by 10% and click Enter (see Figure below).Now click the “Copy To” icon to copy the value from April across the a. periods till December (see Figure below)After clicking the Copy to icon, the Input Editor screen looks a. similar to the Figure shown below. "
  },
  {
    title:
    ...
    pageTitle: "Budgeting & allocation scenarios",
    url:
      "docs/planning/budget-allocation#scenario-3-to-increase-other-costs-based-on-a-monthly-trend-of-2",
    content:
      "For our example, select the Node 'Other Costs' for Jan month (see a. Figure below).Now click the 'Trend' icon (see Figure below)Now the Trend pop up opens. For our example, select the 'By a. Percentage' option and enter value 2 in the Increase Trend section. b. Now select the Overwrite Values option and click 'Apply' (see Figure c. below).Based on the above configuration, you will be able to view the Input a. Editor screen as shown below. "
  },
  {
    title:
    ...
    pageTitle: "Budgeting & allocation scenarios",
    url:
      "docs/planning/budget-allocation#scenario-4-to-redistribute-support-costs-based-on-cost-of-sales",
    content:
      "For our example, select the Node 'Support Costs' in Total column (see Figure below).Now click the 'Weight' icon (see Figure below)The Spread by Weight pop up opens. For our example, select the Node as 'Cost of Sales'. Select the 'Overwrite Values' option and click 'Apply' (see Figure below).As an end result of the Scenario 4, you will be able to view the Input Editor screen loaded with the redistributed values (see Figure below). "
  },
  {
    title:
